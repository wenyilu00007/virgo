...
## 项目介绍
   - 项目生成 ID 核心逻辑由SnowflakeIdWorker.java完成
   - 接口文档可以查看/virgo/swagger-ui.html
   - 启动命令
  ```
       带日志java ：-jar virgo-1.0-SNAPSHOT.jar --workid=0 --datacenterid=0 
       不带日志后台运行 ：java -jar virgo-1.0-SNAPSHOT.jar --workid=0 --datacenterid=0 >>/dev/null &
  ```

   - 启动命令中必须指定 workid 及 datacenterid,否则项目不能启动（报错信息如下），
     并且确保 workid+datacenterid 唯一，否则会导致生成ID可能重复
``` 
